	...
	namespace tz
	{
		...
		const Zone* const ZoneGroup::FindActiveZone(BasicDateTime<> cur_dt, Choose choose)
		{
			...
			if (cur_dt.getType() == KTimeType_Wall)
			{
				...
				{
					cur_zone_index = i;
					if (cur_dt.getRd() < zone_arr_[i].until_wall || AlmostEqualUlps(cur_dt.getRd(), zone_arr_[i].until_wall, 11))
						return &zone_arr_[i];
				}
			}
			else if (cur_dt.getType() == KTimeType_Utc)
			{
				...
				{
					cur_zone_index = i;
					if (cur_dt.getRd() < zone_arr_[i].until_utc || AlmostEqualUlps(cur_dt.getRd(), zone_arr_[i].until_utc, 11))
						return &zone_arr_[i];
				}
			}
		...
		const Zone* const ZoneGroup::CorrectForAmbigWallOrUtc(const BasicDateTime<>& cur_dt, int cur_zone_index, Choose choose)
		{
			...
			if (prev_zone)
			{
				...
				if ((cur_dt.getRd() >= cur_zone_rd || AlmostEqualUlps(cur_dt.getRd(), cur_zone_rd, 11)) && cur_dt.getRd() < cur_zone_inst)
				{
			...
			if (next_zone)
			{
				...
				if (cur_dt.getType() == KTimeType_Wall)
				{
					...
					if ((cur_dt.getRd() >= next_zone_inst || AlmostEqualUlps(cur_dt.getRd(), next_zone_inst, 11)) && cur_dt.getRd() < next_zone_rd)
					{
