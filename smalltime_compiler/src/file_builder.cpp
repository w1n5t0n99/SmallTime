...
namespace smalltime
{
	namespace comp
	{
			...
			std::vector<tz::Rules>& vec_rule_lookup, std::ofstream& out_file)
		{
			...
			tz::Zone z;
			size_t total_zone_size = (sizeof(z.abbrev) + sizeof(z.mb_rule_offset) + sizeof(z.mb_until_utc) + sizeof(z.next_zone_offset) +
				sizeof(z.rule_id) + sizeof(z.trans_rule_offset) + sizeof(z.until_type) + sizeof(z.zone_id) + sizeof(z.zone_offset)) * vec_zone.size();

			tz::Rule r;
			size_t total_rule_size = (sizeof(r.at_time) + sizeof(r.at_type) + sizeof(r.day) + sizeof(r.day_type) +
				sizeof(r.from_year) + sizeof(r.letter) + sizeof(r.month) + sizeof(r.offset) + sizeof(r.rule_id) + sizeof(r.to_year)) * vec_rule.size();

			tz::Zones zs;
			size_t total_zones_size = (sizeof(zs.first) + sizeof(zs.size) + sizeof(zs.zone_id)) * vec_zone_lookup.size();


			tz::Zones rs;
			size_t total_rules_size = (sizeof(rs.first) + sizeof(rs.size) + sizeof(rs.zone_id)) * vec_rule_lookup.size();

			size_t tzdb_file_size = (sizeof(size_t) * 5) + sizeof(tzdb_id) + total_zone_size + total_rule_size + total_zones_size + total_rules_size;
